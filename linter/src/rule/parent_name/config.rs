...
impl Config {
    pub(super) fn compile(self) -> Result<Vec<Assertion>, Error> {
        self.0.into_iter().enumerate().map(|(index, value)| {
            ...
            if value.ignored_names.iter().any(|name| name.is_empty() || name.contains(['/', '\\']) || matches!(name.as_str(), "." | "..")) {
                return Err(Error::Configuration(format!("{setting}.ignored_names: expected nonempty file stems, without path separators")));
            }
            Ok(Assertion {
                selector: value.target.compile(&format!("{setting}.target"), true)?,
                exclude: value.exclude.map(|target| target.compile(&format!("{setting}.exclude"), true)).transpose()?,
