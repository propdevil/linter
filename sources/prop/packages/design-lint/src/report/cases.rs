...
/// Flat Markdown review queues under a root directory: `errors/` (violations
/// to fix) and `check/` (classified "review later" findings). A case file is
/// written for every violation and every reviewed finding, so the queue is the
/// agent's todo list across ALL enabled rules — not just the free-function
/// classify workflow.
...
impl<Output> Cases<Output> {
    ...
    fn queue(&self, finding: &Finding) -> Option<(PathBuf, String)> {
        match finding.review.as_ref().map(|review| &review.state) {
            Some(ReviewState::Error) => Some((self.root.join("errors"), "unclassified".to_owned())),
            Some(ReviewState::Check(classification)) => {
                Some((self.root.join("check"), classification.clone()))
            }
            // Rules without the classify workflow (async-blocking, file-length,
            // ...) still queue every violation into errors/ as a todo item.
            None if finding.is_violation() => {
                Some((self.root.join("errors"), "unclassified".to_owned()))
            }
            None => None,
        }
    }
...
fn document(
    ...
    let metadata = finding
        ...
        .unwrap_or_default();
    let dependencies = match finding.review.as_ref() {
        Some(review) if !review.dependencies.is_empty() => review
            .dependencies
            .iter()
            .map(|dependency| format!("- `{dependency}`"))
            .collect::<Vec<_>>()
            .join("\n"),
        _ => "- None detected".to_owned(),
    };
    let questions = match finding.review.as_ref() {
        Some(review) if !review.questions.is_empty() => review
            .questions
            .iter()
            .map(|question| format!("- {question}"))
            .collect::<Vec<_>>()
            .join("\n"),
        _ => "- Confirm whether the finding represents the intended design.".to_owned(),
    };
    let related = finding
        .related
        .iter()
