...
pub(super) fn findings(workspace: &Workspace, policy: &Policy) -> Vec<Finding> {
    workspace
        .production()
        .filter(|source| !required_module(source, workspace, policy))
        .filter_map(|source| candidate(source, workspace))
        .collect()
}

fn required_module(source: &SourceFile, workspace: &Workspace, policy: &Policy) -> bool {
    workspace.packages.iter().any(|package| {
        let Some(parent) = package.root.parent() else {
            return false;
        };
        let Some(name) = package.root.file_name().and_then(|name| name.to_str()) else {
            return false;
        };
        if !parent.ends_with(&policy.repository.chain_root)
            || policy
                .repository
                .chain_exclusions
                .iter()
        ...
        policy
            ...
            .chain_skeleton
            .iter()
            .any(|required| source.path == package.root.join(required))
            || policy
                .repository
                .chain_directories
                .iter()
                .any(|required| source.path == package.root.join(required).join("mod.rs"))
    })
}

fn candidate(source: &SourceFile, workspace: &Workspace) -> Option<Finding> {
...
fn child_path(source: &SourceFile, child: &ItemMod) -> Option<PathBuf> {
...
fn has_qualified_use(source: &SourceFile, module: &str) -> bool {
    struct Paths<'a> {
        module: &'a str,
        found: bool,
    }
