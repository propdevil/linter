...
use syn::{
    BinOp, Expr, ExprAssign, ExprBinary, ExprField, ExprMatch, ExprMethodCall, ExprPath,
    ExprStruct, Fields, ImplItemFn, ItemFn, ItemImpl, ItemMod, ItemStruct, Member, visit::Visit,
};
...
pub(crate) fn check(workspace: &Workspace, _policy: &Policy) -> Result<Vec<Finding>> {
    ...
    Ok(concepts
        .into_iter()
        .filter_map(|(concept, evidence)| finding(ID, concept, evidence))
        .collect())
}
