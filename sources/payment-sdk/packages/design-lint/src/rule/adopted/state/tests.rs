use crate::{model::Finding, policy::Policy, test_support::Fixture};
...
#[test]
fn sdk_confirmation_status_keeps_warning_and_literal_evidence() {
    let values = findings(
        r#"
struct Transaction { status: String }
impl Transaction {
...
"#,
    ...
    assert_eq!(values[0].subject, "Transaction::status");
    assert_eq!(values[0].location.line, 6);
    assert_eq!(values[0].related.len(), 2);
    assert!(
        values[0]
            .review
            .as_ref()
            ...
            .metadata
            .iter()
            .any(|(key, value)| key == "Values" && value.contains("confirmed"))
    );
}
