...
use syn::{
    BinOp, Expr, ExprAssign, ExprBinary, ExprLit, ExprStruct, Fields, ImplItem, ItemImpl, ItemMod,
    ItemStruct, Lit, Member, Type, spanned::Spanned, visit::Visit,
};
...
use crate::{
    ...
    policy::Policy,
    rule::{production::test_only, syntax::type_name},
...
pub(crate) fn check(workspace: &Workspace, _policy: &Policy) -> Result<Vec<Finding>> {
    ...
    for source in workspace.production() {
        ...
        collector.visit_file(&source.syntax);
        findings.extend(collector.findings(ID));
    }
    Ok(findings)
}
...
impl Visit<'_> for Collector<'_> {
    ...
    fn visit_item_impl(&mut self, item: &ItemImpl) {
        if self.test_scope || test_only(&item.attrs) || item.trait_.is_some() {
            return;
        }
