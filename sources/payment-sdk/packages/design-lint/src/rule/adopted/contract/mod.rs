...
use syn::{
    Attribute, FnArg, ItemImpl, ItemMod, ItemTrait, ReturnType, TraitItem, TraitItemFn,
...
pub(crate) const ID: &str = "broad-trait-responsibilities";
    ...
    Ok(definitions
        ...
        .filter_map(|definition| {
            let implementation = implementations
                ...
                .cloned()
                .unwrap_or_default();
            definition.finding(ID, implementation)
        })
        .collect())
}
...
impl Visit<'_> for Collector<'_> {
    fn visit_item_mod(&mut self, item: &ItemMod) {
        ...
        self.test_scope |= test_only(&item.attrs);
    ...
    fn visit_item_trait(&mut self, item: &ItemTrait) {
        if self.test_scope || test_only(&item.attrs) || excluded_trait(item, self.source) {
            return;
        }
        let methods = item
            .items
            ...
            .filter_map(|item| match item {
                TraitItem::Fn(item) if !test_only(&item.attrs) => Some(method(item, self.source)),
                _ => None,
            })
            .collect::<Vec<_>>();
        ...
        self.definitions.push(Definition {
            ...
            name,
            location: self.source.location(item.ident.span()),
            methods,
        });
    }
...
fn method(method: &TraitItemFn, source: &SourceFile) -> Method {
...
fn normalized(source: &SourceFile, span: proc_macro2::Span) -> String {
    source
        .excerpt(span)
        .split_whitespace()
        .collect::<Vec<_>>()
...
fn excluded_trait(item: &ItemTrait, source: &SourceFile) -> bool {
...
fn type_words(source: &SourceFile, span: proc_macro2::Span) -> impl Iterator<Item = String> {
...
fn words(name: &str) -> impl Iterator<Item = String> + use<> {
