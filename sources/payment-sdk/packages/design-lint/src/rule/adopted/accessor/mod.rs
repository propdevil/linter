...
use quote::ToTokens;
use syn::{
    Expr, FnArg, ImplItem, Item, ItemImpl, ItemStruct, Member, Pat, Stmt, Visibility,
    spanned::Spanned,
};
...
use crate::rule::syntax::type_name;
...
impl Definitions {
    ...
    fn insert(&mut self, source: &SourceFile, item: &ItemStruct) {
...
fn inspect_source(source: &SourceFile, definitions: &Definitions, findings: &mut Vec<Finding>) {
    for item in &source.syntax.items {
