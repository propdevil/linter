...
#[test]
fn reports_accessors_over_already_public_fields() {
...
#[test]
fn reports_an_exact_duplicate_private_field_accessor_as_review() {
    let findings = findings(
        r#"
pub struct Image {
    reference: String,
...
#[test]
fn preserves_deliberate_private_field_boundaries() {
    let findings = findings(
        r#"
pub struct Account {
    balance: i64,
...
#[test]
fn documentation_does_not_hide_redundant_public_access() {
    let findings = findings(
        r#"
/// Plain data.
pub struct Data {
...
#[test]
fn ignores_computed_converted_and_derived_values() {
    let findings = findings(
        r#"
pub struct Value {
    pub bytes: Vec<u8>,
...
#[test]
fn ignores_trait_methods_and_compatibility_markers() {
    let findings = findings(
        r#"
...
#[test]
fn ignores_wire_and_ffi_models() {
    let findings = findings(
        r#"
...
#[test]
fn does_not_compare_different_forwarding_shapes_as_duplicates() {
    let findings = findings(
        r#"
...
#[test]
fn does_not_compare_distinct_borrowed_view_contracts_as_duplicates() {
    let findings = findings(
        r#"
...
#[test]
fn sdk_wallet_address_keeps_private_representation_boundary() {
    let values = findings(
        r#"
pub struct Wallet { address: Address, pub id: WalletId }
impl Wallet {
...
"#,
    ...
    assert_eq!(values[0].subject, "Wallet::id");
    assert_eq!(values[0].severity, crate::Severity::Warning);
    assert_eq!(values[0].related.len(), 1);
    assert!(values[0].related[0].label.contains("exposed"));
}
