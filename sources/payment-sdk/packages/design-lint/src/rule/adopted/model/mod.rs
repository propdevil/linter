...
use quote::ToTokens;
use syn::{
    Attribute, Fields, GenericArgument, ImplItem, ItemImpl, ItemMod, ItemStruct, ItemType,
    PathArguments, Type, Visibility, spanned::Spanned, visit::Visit,
};
        ...
        dependencies: workspace
            ...
            .flat_map(|package| {
                package
                    .dependencies
                    .iter()
                    .map(|dependency| (package.name.clone(), dependency.clone()))
            })
            .collect(),
        ..Database::default()
    };
...
impl<'a> Collector<'a> {
    fn new(source: &'a SourceFile, aliases: HashMap<String, String>) -> Self {
        Self {
            source,
            test_scope: false,
            aliases,
...
impl Visit<'_> for Collector<'_> {
    ...
    fn visit_item_impl(&mut self, item: &ItemImpl) {
        ...
        let behavior = item
            .items
            .iter()
            .filter(|member| matches!(member, ImplItem::Fn(method) if !test_only(&method.attrs)))
            .count();
