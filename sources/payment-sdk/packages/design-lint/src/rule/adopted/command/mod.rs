...
use syn::{
    Block, Expr, ExprCall, ExprMacro, ExprMethodCall, ImplItemFn, ItemFn, ItemMod, ItemUse, Local,
    Pat, UseTree, spanned::Spanned, visit::Visit,
};
...
pub(crate) fn check(workspace: &Workspace, policy: &Policy) -> Result<Vec<Finding>> {
    ...
    for source in workspace.sources() {
        ...
        let mut visitor = Commands::new(source, allowed);
...
impl<'ast> Visit<'ast> for Commands<'_> {
    ...
    fn visit_item_mod(&mut self, module: &'ast ItemMod) {
        ...
        let test = crate::rule::production::test_only(&module.attrs);
        self.test_depth += usize::from(test);
